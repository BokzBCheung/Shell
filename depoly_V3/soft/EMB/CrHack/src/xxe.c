...
/* 解码一个字符 */
#define XX_DEC(c)   (table[(c)])

/* 编码一个字符 */
#define XX_ENC(c)   (s_xxe[(c) & 077])

/* 常数查找表 */
static const ansi_t _rom_ s_xxe[] =
    ...
    )
{
    ...
    c1 = (XX_DEC(in[0]) << 2) | (XX_DEC(in[1]) >> 4);
    c2 = (XX_DEC(in[1]) << 4) | (XX_DEC(in[2]) >> 2);
    c3 = (XX_DEC(in[2]) << 6) | (XX_DEC(in[3]) >> 0);
    if (len >= 1) out[0] = c1;
    if (len >= 2) out[1] = c2;
    if (len >= 3) out[2] = c3;
    return ((len >= 3) ? 3 : len);
    ...
    )
{
    ...
    for (idx = 0; idx < len; idx++, i_ptr += 45) {
        if (o_ptr >= dstlen)
            return (0);
        o_buf[o_ptr++] = XX_ENC(45);
        for (ii = 0; ii < 45; ii += 3) {
            if (o_ptr + 3 >= dstlen)
                return (0);
            xx_outenc(&i_buf[i_ptr + ii], &o_buf[o_ptr]);
            o_ptr += 4;
        }
    ...
    )
{
    ...
    {
        /* 跳过非法字符 */
        if (!xx_is_okay(i_buf[i_ptr])) {
            i_ptr += 1;
            continue;
        }

        /* 读取长度字符 */
        idx = XX_DEC(i_buf[i_ptr]);
        if (idx == 0)
            break;
        if (++i_ptr >= srclen)
            return (0);
        ...
        for (ii = 0; ii < len; ii += 4)
        {
            ...
            jj = xx_outdec(table, in, &o_buf[o_ptr], idx);
            idx   -= jj;
            o_ptr += jj;
        }
    }
