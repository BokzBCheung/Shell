...
/* BLOWFISH 初始 PBOX 和 SBOX 常数 */
static const int32u _rom_ s_pbox[18] =
{
...
static const int32u _rom_ s_sbox[1024] =
{
...
/* BLOWFISH 计算宏 */
#define BF_GET32_3(x)   (((x) >>  0) & 0xFF)
#define BF_GET32_2(x)   (((x) >>  8) & 0xFF)
#define BF_GET32_1(x)   (((x) >> 16) & 0xFF)
#define BF_GET32_0(x)   (((x) >> 24) & 0xFF)

#define BF_F(x) (((ss[  0 + BF_GET32_0(x)]  + ss[256 + BF_GET32_1(x)]) \
                 ^ ss[512 + BF_GET32_2(x)]) + ss[768 + BF_GET32_3(x)])

#define BF_ROUND(a,b,n)     (b) ^= pp[n]; (a) ^= BF_F(b)

/*
---------------------------------------
    BLOWFISH 加密数据块 (字节顺序相关)
---------------------------------------
*/
static void_t
bf_encrypt_block (
  __CR_IN__ const sBF*      ctx,
    ...
    )
{
    int32u          yl = src[0];
    int32u          yr = src[1];
    const int32u*   pp = ctx->p;
    const int32u*   ss = ctx->s;
    ...
    )
{
    ...
    for (pos = 0, ii = 0; ii < 18; ii++) {
        temp = ((int32u)(((byte_t*)key)[(pos + 0)         ]) << 24) |
               ((int32u)(((byte_t*)key)[(pos + 1) % keylen]) << 16) |
               ((int32u)(((byte_t*)key)[(pos + 2) % keylen]) <<  8) |
               ((int32u)(((byte_t*)key)[(pos + 3) % keylen]));
        pp[ii] ^= temp;
        pos = (pos + 4) % keylen;
    }
    ...
    for (ii = 0; ii < 4; ii++)
    for (jj = 0, count = ii * 256; jj < 256; jj += 2, count += 2)
    {
        bf_encrypt_block(ctx, data, data);
        ss[count + 0] = data[0];
        ss[count + 1] = data[1];
    ...
    )
{
    ...
    i32[0] = DWORD_BE(src[0]);
    i32[1] = DWORD_BE(src[1]);
    bf_encrypt_block(ctx, o32, i32);
    dst[0] = DWORD_BE(o32[0]);
    dst[1] = DWORD_BE(o32[1]);
}
    ...
    )
{
    const int32u*   pp = ctx->p;
    const int32u*   ss = ctx->s;
    int32u  yl = DWORD_BE(src[0]);
    int32u  yr = DWORD_BE(src[1]);
    ...
    dst[0] = DWORD_BE(yr ^ pp[0]);
    dst[1] = DWORD_BE(yl ^ pp[1]);
}
