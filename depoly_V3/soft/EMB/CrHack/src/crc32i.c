...
/* CRC-32/IEEE 常数表 (X^32 + X^26 + X^23 + X^22 + X^16 +
...
{
    int32u  hash = hash_crc32i_init();

    hash = hash_crc32i_update(hash, data, size);

    return (hash_crc32i_finish(hash));
}
